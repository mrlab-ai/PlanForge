...
fn get_axioms_by_atom(
    axioms: &[PropositionalAxiom],
) -> HashMap<(String, Vec<String>), Vec<PropositionalAxiom>> {
    let mut axioms_by_atom: HashMap<(String, Vec<String>), Vec<PropositionalAxiom>> =
        HashMap::new();
    for axiom in axioms {
...
fn compute_necessary_axiom_literals(
    ...
    let register_literals = |literals: &[Literal],
                             negated: bool,
                             necessary: &mut HashSet<Literal>,
                             q: &mut Vec<Literal>| {
...
fn condition_to_literals(cond: &crate::translate::pddl::Condition) -> Vec<Literal> {
    ...
    match cond {
        ...
        Condition::Not(inner) => match inner.as_ref() {
            ...
            _ => condition_to_literals(inner)
                .into_iter()
                .map(|l| l.negate())
                .collect(),
        },
        Condition::And(parts) => parts.iter().flat_map(condition_to_literals).collect(),
        Condition::Or(parts) => parts.iter().flat_map(condition_to_literals).collect(),
        Condition::Forall(_, inner) | Condition::Exists(_, inner) => condition_to_literals(inner),
...
fn simplify(axioms: &[PropositionalAxiom]) -> Vec<PropositionalAxiom> {
    ...
    let processed_axioms: Vec<PropositionalAxiom> = axioms
        .iter()
        .map(|ax| {
...
fn negate(axioms: &[PropositionalAxiom]) -> Vec<PropositionalAxiom> {
    ...
    let mut result: Vec<PropositionalAxiom> = vec![PropositionalAxiom::new(
        axioms[0].name.clone(),
        vec![],
...
fn compute_axiom_layers(
    ...
    let mut depends_on: HashMap<(String, Vec<String>), HashSet<((String, Vec<String>), i32)>> =
        HashMap::new();
