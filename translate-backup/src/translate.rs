...
use super::normalize;
use super::numeric_axiom_rules::{InstantiatedNumericAxiom, PrimitiveNumericExpression};
use super::options;
use super::pddl;
use super::pddl::{Atom, FunctionComparison, Literal, NegatedFunctionComparison};
use super::pddl_parser::PddlTask;
use super::sas as internal_sas;
use super::sas_tasks as py_sas_tasks;
use super::simplify;
...
pub fn translate_from_files_with_config(
    ...
    let task = PddlTask::from_files(domain, problem)
        .map_err(|err: anyhow::Result::Err| err.to_string())?;
...
pub fn translate_from_ast(
    ...
    let exploration = instantiate::explore_normalized(&norm_task).map_err(|err| err.to_string())?;
...
fn ensure_expr_var_visit(
    sexpr: &crate::pddl_parser::SExpr,
    df_admin: &mut crate::derived_function_admin::DerivedFunctionAdministrator,
    num_index: &mut std::collections::HashMap<String, usize>,
    numeric_list: &mut Vec<crate::sas::NumericVariable>,
    numeric_init_vec: &mut Vec<f64>,
    instantiated_num_axioms: &mut Vec<crate::numeric_axiom_rules::InstantiatedNumericAxiom>,
    ...
    match sexpr {
        crate::pddl_parser::SExpr::List(inner) => {
            if inner.is_empty() {
                return None;
            }
            if let crate::pddl_parser::SExpr::Atom(op) = &inner[0] {
                if op == "+" || op == "-" || op == "*" || op == "/" {
                    let pne = df_admin
                        .get_derived_function(&crate::pddl_parser::SExpr::List(inner.clone()));
                    let derived_key = format_pne_key(&pne.name, &pne.args);
                    let mut parts_numericparts: Vec<crate::numeric_axiom_rules::NumericPart> =
                        Vec::new();
                    for p in &inner[1..] {
                        match p {
                            crate::pddl_parser::SExpr::Atom(a) => {
                                if let Ok(nv) = a.parse::<f64>() {
                                    parts_numericparts.push(
                                        crate::numeric_axiom_rules::NumericPart::Constant(
                                            crate::numeric_axiom_rules::NumericConstant(
                                    ...
                                    let prim =
                                        crate::numeric_axiom_rules::PrimitiveNumericExpression {
                                            name: a.clone(),
                                            args: vec![],
                                        };
                                    parts_numericparts.push(
                                        crate::numeric_axiom_rules::NumericPart::Primitive(prim),
                                    );
                                }
                            }
                            crate::pddl_parser::SExpr::List(_) => {
                                let child = df_admin.get_derived_function(p);
                                let prim = crate::numeric_axiom_rules::PrimitiveNumericExpression {
                                    name: child.name.clone(),
                                    args: child.args.clone(),
                                };
                                parts_numericparts
                                    .push(crate::numeric_axiom_rules::NumericPart::Primitive(prim));
                            }
                        }
                    }
                    if !num_index.contains_key(&derived_key) {
                        ...
                        numeric_list.push(crate::sas::NumericVariable {
                        ...
                        let effect = crate::numeric_axiom_rules::PrimitiveNumericExpression {
                            name: pne.name.clone(),
                            args: pne.args.clone(),
                        };
                        let ax = crate::numeric_axiom_rules::InstantiatedNumericAxiom {
                            name: derived_key.clone(),
                            op: Some(op.clone()),
                ...
                if let crate::pddl_parser::SExpr::Atom(fname) = &inner[0] {
                    let args = inner[1..]
                        .iter()
                        .filter_map(|x| match x {
                            crate::pddl_parser::SExpr::Atom(a) => Some(a.clone()),
                            _ => None,
                        })
                        .collect::<Vec<_>>();
        ...
        crate::pddl_parser::SExpr::Atom(a) => {
            if let Ok(v) = a.parse::<f64>() {
                let const_symbol =
                    format!("derived!{}", crate::function_expression::format_float(v));
                ...
                if !num_index.contains_key(&const_key) {
                    ...
                    let pne = crate::numeric_axiom_rules::PrimitiveNumericExpression {
                        name: const_symbol.clone(),
                        args: vec![],
                    };
                    let part = crate::numeric_axiom_rules::NumericPart::Constant(
                        crate::numeric_axiom_rules::NumericConstant(ordered_float::OrderedFloat(v)),
                    );
                    let ax = crate::numeric_axiom_rules::InstantiatedNumericAxiom {
                        name: const_key.clone(),
                        op: None,
...
pub fn translate_task_from_grounded_internal(
    ops: &[crate::instantiate::GroundedOp],
    dom: &crate::pddl::Domain,
    prob: &crate::pddl::Problem,
    external_instantiated_num_axioms: &Vec<crate::numeric_axiom_rules::InstantiatedNumericAxiom>,
    py_groups: Option<Vec<Vec<String>>>,
    grounded_axioms: &[crate::instantiate::GroundedAxiom],
    normalized_goal: &crate::pddl::Condition,
    norm_task: &crate::normalize::NormalizableTask,
    ...
    let mut vars: Vec<crate::sas::Variable> = Vec::new();
    let mut ranges: Vec<usize> = Vec::new();
    let mut fact_to_varvals: HashMap<String, Vec<(usize, usize)>> = HashMap::new();
    let mut instantiated_num_axioms: Vec<crate::numeric_axiom_rules::InstantiatedNumericAxiom> =
        external_instantiated_num_axioms.clone();
    let mut derived_axiom_index: std::collections::HashMap<String, usize> =
        std::collections::HashMap::new();

    let mut df_admin = crate::derived_function_admin::DerivedFunctionAdministrator::new();
    let mut ensure_expr_var =
        |sexpr: &crate::pddl_parser::SExpr,
         ...
         instantiated_num_axioms: &mut Vec<
            crate::numeric_axiom_rules::InstantiatedNumericAxiom,
        >,
         derived_axiom_index: &mut std::collections::HashMap<String, usize>|
         -> Option<String> {
    ...
    for act in &dom.actions {
        if let Some(eff_s) = &act.effect {
            let eff = crate::pddl::sexpr_to_effect(eff_s);
            fn collect(e: &crate::pddl::Effect, set: &mut std::collections::HashSet<String>) {
                match e {
                    crate::pddl::Effect::Add(n, _) | crate::pddl::Effect::Del(n, _) => {
                        set.insert(n.clone());
                    }
                    crate::pddl::Effect::And(v) => {
                        for sub in v {
                            collect(sub, set);
                        }
                    }
    ...
    for a in &prob.init {
        if let crate::pddl_parser::SExpr::List(list) = a {
            if list.len() >= 3 {
                if let crate::pddl_parser::SExpr::Atom(op) = &list[0] {
                    if op == "=" {
                        if let crate::pddl_parser::SExpr::List(left) = &list[1] {
                            if let crate::pddl_parser::SExpr::Atom(fname) = &left[0] {
                                let arg_s = left[1..]
                                    .iter()
                                    .filter_map(|x| match x {
                                        crate::pddl_parser::SExpr::Atom(s) => Some(s.clone()),
                                        _ => None,
                                    })
                                    .collect::<Vec<_>>()
                                    .join(", ");
                                let key = format!("{}({})", fname, arg_s);
                                if let crate::pddl_parser::SExpr::Atom(val) = &list[2] {
                    ...
                    } else if let crate::pddl_parser::SExpr::Atom(name) = &list[0] {
                        if fluent_preds.contains(name.as_str()) {
                            let atom = format!(
                                ...
                                list[1..]
                                    .iter()
                                    .filter_map(|x| match x {
                                        crate::pddl_parser::SExpr::Atom(s) => {
                                            Some(s.clone())
                                        }
                                        _ => None,
                                    })
    ...
    for op in ops {
        if !op.effects.is_empty() {
            for (_conds, eff) in &op.effects {
                match eff {
                    crate::pddl::Effect::Add(name, args) => {
                        grounded_atoms.push(format!("{}({})", name, args.join(", ")));
                    }
                    crate::pddl::Effect::Del(name, args) => {
                        grounded_atoms.push(format!("{}({})", name, args.join(", ")));
                    }
                    crate::pddl::Effect::Increase(name, args, val) => {
                        let key = format!("{}({})", name, args.join(", "));
                        numeric_vars.push((key, *val));
                    }
                    crate::pddl::Effect::Decrease(name, args, val) => {
                        let key = format!("{}({})", name, args.join(", "));
                        numeric_vars.push((key, -*val));
                    }
                    crate::pddl::Effect::And(_) => {}
                }
            }
            ...
            match eff {
                ...
                crate::pddl::Effect::Increase(name, args, v) => {
                    let key = format!("{}({})", name, args.join(", "));
                    numeric_vars.push((key, *v));
                }
                crate::pddl::Effect::Decrease(name, args, v) => {
                    let key = format!("{}({})", name, args.join(", "));
                    numeric_vars.push((key, -*v));
                }
            }
        ...
        if let Some(pre) = &op.pre {
            match pre {
                crate::pddl::Condition::Atom(name, args) => {
                    if fluent_preds.contains(name.as_str()) {
                        grounded_atoms.push(format!("{}({})", name, args.join(", ")));
                    }
                }
                crate::pddl::Condition::And(v) => {
                    for c in v {
                        match c {
                            crate::pddl::Condition::Atom(name, args) => {
                                if fluent_preds.contains(name.as_str()) {
                            ...
                            crate::pddl::Condition::Comparison(_, _, _) => {}
                            crate::pddl::Condition::Not(_) => {}
                            crate::pddl::Condition::And(_) => {}
                            crate::pddl::Condition::Or(_) => {}
                            crate::pddl::Condition::Forall(_, _) => {}
                            crate::pddl::Condition::Exists(_, _) => {}
                            crate::pddl::Condition::True => {}
                        }
                    }
                }
                crate::pddl::Condition::Comparison(_, _, _) => {}
    ...
    fn collect_atoms_from_condition(
        ...
        match cond {
            ...
            crate::pddl::Condition::And(parts) | crate::pddl::Condition::Or(parts) => {
                for part in parts {
                    collect_atoms_from_condition(part, grounded_atoms, fluent_preds);
                }
            }
            crate::pddl::Condition::Not(inner)
            | crate::pddl::Condition::Forall(_, inner)
            | crate::pddl::Condition::Exists(_, inner) => {
    ...
    let (chosen_groups, _mutex_groups, translation_key) = if let Some(pg) = py_groups {
        ...
        crate::fact_groups::compute_groups(norm_task, &grounded_atoms, None)
    };
    ...
    let mut numeric_list: Vec<crate::sas::NumericVariable> = Vec::new();
    ...
    let mut pre_num_map: std::collections::HashMap<String, f64> = std::collections::HashMap::new();
    ...
    let constant_axioms =
        crate::numeric_axiom_rules::identify_constants_inplace(&mut instantiated_num_axioms);
    let axiom_by_pne = crate::numeric_axiom_rules::axiom_by_pne(&instantiated_num_axioms);
    let constant_effects: std::collections::HashSet<_> =
        constant_axioms.iter().map(|a| &a.effect).collect();
    let (axioms_by_layer, _max_layer) = crate::numeric_axiom_rules::compute_axiom_layers(
        &instantiated_num_axioms,
        &constant_axioms,
        &axiom_by_pne,
    );
    ...
    let axiom_map =
        crate::numeric_axiom_rules::identify_equivalent_axioms(&axioms_by_layer, &axiom_by_pne);
    ...
    let metric_idx: isize = match &norm_task.metric.1 {
        None => *num_index
            .get("total-cost()")
            .ok_or_else(|| "metric fluent not found: total-cost()".to_string())?
            as isize,
        Some(crate::pddl_parser::SExpr::Atom(name)) if name == "-1" => *num_index
            .get("total-cost()")
            .ok_or_else(|| "metric fluent not found: total-cost()".to_string())?
            as isize,
        Some(expr) => {
            let metric_key = match expr {
                crate::pddl_parser::SExpr::Atom(name) => format!("{}()", name),
                crate::pddl_parser::SExpr::List(items) => {
                    if items.is_empty() {
                        return Err("metric expression is empty".to_string());
                    }
                    let name = match &items[0] {
                        crate::pddl_parser::SExpr::Atom(n) => n.clone(),
                        _ => return Err("metric expression has no name".to_string()),
                    };
                    let args: Vec<String> = items[1..]
                        .iter()
                        .filter_map(|s| {
                            if let crate::pddl_parser::SExpr::Atom(a) = s {
                                Some(a.clone())
                            } else {
                                None
                            }
    ...
    let mut operators: Vec<crate::sas::SASOperator> = Vec::new();
    let mut numeric_axioms: Vec<crate::sas::NumericAxiom> = Vec::new();
    let mut comp_axioms: Vec<crate::sas::CompareAxiom> = Vec::new();
    ...
    for op in ops {
                ...
                crate::pddl::Condition::And(v) => {
                    for c in v {
                        match c {
                            ...
                            crate::pddl::Condition::Comparison(opstr, l, r) => {
                                ...
                                if let (Some(left_key), Some(right_key)) = (left_name, right_name) {
                                        ...
                                        (num_index.get(&left_key), num_index.get(&right_key))
                                    {
                                        ...
                                        if !comp_axiom_added.contains(&effect_idx) {
                                            ...
                                            comp_axioms.push(crate::sas::CompareAxiom {
        ...
        if !op.effects.is_empty() {
            for (conds, eff) in &op.effects {
                let condition = {
                    let mut build_condition_vals = |conds: &[crate::pddl::Condition]| {
                        let mut cond_vals: Vec<(usize, usize)> = Vec::new();
                        let mut stack: Vec<crate::pddl::Condition> = conds.to_vec();
                        while let Some(cond) = stack.pop() {
                            match cond {
                                ...
                                crate::pddl::Condition::Not(inner) => {
                                    if let crate::pddl::Condition::Atom(name, args) = *inner {
                                        let atom = format!("{}({})", name, args.join(", "));
                                ...
                                crate::pddl::Condition::And(parts) => {
                                    for part in parts {
                                        stack.push(part);
                                    }
                                }
                                crate::pddl::Condition::Comparison(opstr, l, r) => {
                                        ...
                                        (left_name, right_name)
                                    {
                                            ...
                                            (num_index.get(&left_key), num_index.get(&right_key))
                                        {
                                            ...
                                            let comp_key = format!("{} {} {}", op_norm, ni, nj);
                                            let effect_idx =
                                                if let Some(&ei) = var_index.get(&comp_key) {
                                                    ei
                                                } else {
                                                    let ei = vars.len();
                                                    var_index.insert(comp_key.clone(), ei);
                ...
                match eff {
                    crate::pddl::Effect::Add(name, args) => {
                        ...
                        if let Some(&(v, val)) = atom_to_fdr.get(&atom) {
                            ...
                            let entry = effect_acc.entry((v, cond_key)).or_insert_with(|| {
                                EffectAccumulator {
                    ...
                    crate::pddl::Effect::Increase(nname, args, val) => {
                        let nkey = format!("{}({})", nname, args.join(", "));
                        if let Some(&ni) = num_index.get(&nkey) {
                            let const_name = format!(
                                "derived!{}()",
                                crate::function_expression::format_float(*val)
                            );
                    ...
                    crate::pddl::Effect::Decrease(nname, args, val) => {
                        let nkey = format!("{}({})", nname, args.join(", "));
                        if let Some(&ni) = num_index.get(&nkey) {
                            let const_name = format!(
                                "derived!{}()",
                        ...
                        match sub {
                            crate::pddl::Effect::Add(name, args) => {
                                ...
                                if let Some(&(v, val)) = atom_to_fdr.get(&atom) {
                                    let (cond_key, normalized_cond) = normalize_condition(&[]);
                                    let entry =
                                        effect_acc.entry((v, cond_key)).or_insert_with(|| {
                                            EffectAccumulator {
                                                condition: normalized_cond,
                                                add: None,
                                                del: None,
        ...
        operators.push(crate::sas::SASOperator {
            name: op.name.clone(),
            prevails,
            effects,
    ...
    let (num_axioms_by_layer, _max_layer, num_axiom_map, _const_num_axioms) =
        crate::numeric_axiom_rules::handle_axioms_checked(&instantiated_num_axioms)
            ...
            if let Some(axs) = num_axioms_by_layer.get(&layer) {
                for ax in axs {
                    ...
                    for part in &ax.parts {
                        match part {
                            crate::numeric_axiom_rules::NumericPart::Constant(c) => {
                                values.push(c.0.into_inner());
                            }
                            crate::numeric_axiom_rules::NumericPart::Primitive(pne) => {
                                let key = pne_key(&pne.name, &pne.args);
                                if let Some(idx) = num_index.get(&key) {
                                    values.push(numeric_init_vec.get(*idx).copied().unwrap_or(0.0));
                                } else {
                                    ok = false;
                                    break;
                                }
                            }
                            crate::numeric_axiom_rules::NumericPart::Axiom(ref_ax) => {
    ...
    for (_layer, axs) in &num_axioms_by_layer {
        for ax in axs {
            ...
            let is_constant = ax.op.is_none()
                ...
                && matches!(
                    ax.parts[0],
                    crate::numeric_axiom_rules::NumericPart::Constant(_)
                );
            if is_constant {
                continue;
            }
            ...
            for part in &ax.parts {
                let part_key = match part {
                    ...
                    crate::numeric_axiom_rules::NumericPart::Constant(c) => {
                        format!(
                            "derived!{}()",
                            crate::function_expression::format_float(c.0.into_inner())
                        )
                    }
                    crate::numeric_axiom_rules::NumericPart::Axiom(ref_ax) => {
            ...
            if seen_numeric_axioms.insert(key) {
                numeric_axioms.push(crate::sas::NumericAxiom {
                    op,
                    parts: part_indices,
                    effect: effect_idx,
                });
...

    {
        fn ensure_axiom_comparison_vars(
            ...
            ensure_expr_var: &mut dyn FnMut(
                &crate::pddl_parser::SExpr,
                &mut std::collections::HashMap<String, usize>,
                &mut Vec<crate::sas::NumericVariable>,
                &mut Vec<f64>,
                &mut Vec<crate::numeric_axiom_rules::InstantiatedNumericAxiom>,
                &mut std::collections::HashMap<String, usize>,
            ) -> Option<String>,
            pre_num_map: &std::collections::HashMap<String, f64>,
            num_index: &mut std::collections::HashMap<String, usize>,
            ...
            vars: &mut Vec<crate::sas::Variable>,
            ranges: &mut Vec<usize>,
            comp_axioms: &mut Vec<crate::sas::CompareAxiom>,
    ...
    for sexpr in &prob.init {
        if let crate::pddl_parser::SExpr::List(list) = sexpr {
            if list.is_empty() {
                continue;
            }
            if let crate::pddl_parser::SExpr::Atom(pred) = &list[0] {
                if pred == "=" {
                    continue;
                }
    ...
    let (axiom_layers, comparison_axiom_layer, axiom_init_literals) = {
        ...
        let axiom_result = handle_axioms(
            ops,
            &prop_axioms,
            &goal_literals,
            &global_constraint_literal,
        );
    ...
    let canonical_variables: Vec<crate::sas::CanonicalVariable> = vars
        .iter()
        .enumerate()
        .map(|(idx, v)| {
            let axiom_layer = axiom_layers.get(idx).copied().unwrap_or(-1);
            crate::sas::CanonicalVariable {
                name: format!("var{}", idx),
                axiom_layer,
    ...
    let canonical_operators: Vec<crate::sas::CanonicalOperator> = operators
        .iter()
        .map(|op| {
            let pre_post = op
                .effects
                .iter()
                .map(|(var, pre, post, cond)| crate::sas::CanonicalEffect {
                    var: *var,
                    pre: if *pre == usize::MAX { None } else { Some(*pre) },
                    post: *post,
            ...
            let assign_effects = op
                ...
                .iter()
                .map(
                    |(target, assign_op, rhs_var, cond)| crate::sas::CanonicalAssignEffect {
                        target: *target,
                        op: assign_op.clone(),
                        rhs: crate::sas::CanonicalAssignRhs::Variable(*rhs_var),
                        condition: cond.clone(),
                    },
                )
                .collect();
            crate::sas::CanonicalOperator {
                name: op.name.clone(),
    ...
    let mut sas_axioms: Vec<crate::sas::SASAxiom> = Vec::new();
    ...
    for ax in grounded_axioms {
        if let Some(&(effect_var, effect_val)) = atom_to_fdr.get(&ax.effect_atom) {
            ...
            sas_axioms.push(crate::sas::SASAxiom {
...
fn condition_to_axiom_literals(cond: &crate::pddl::Condition) -> Vec<AxiomLiteral> {
    use crate::pddl::Condition;
    ...
    match cond {
        ...
        Condition::And(parts) => parts.iter().flat_map(condition_to_axiom_literals).collect(),
        Condition::Or(parts) => parts.iter().flat_map(condition_to_axiom_literals).collect(),
                    ...
                    if other_prop != prop {
                        if let Some(other_facts) = strips_to_sas.get(other_prop) {
                            for other_fact in other_facts {
                                implied.entry(*other_fact).or_default().push(prop_is_false);
                            }
                        }
                    }
                }
        ...
        match fact {
            Literal::FunctionComparison(comp) => {
                ...
                if !dictionary.contains_key(fact) {
                    ...
                    let parts: Vec<usize> =
                        comp.parts.iter().map(|p| numeric_dictionary[p]).collect();
                    ...
                    if let Some(pos_fact) = comp_axiom_dict.get(&key) {
                        ...
                        let neg_fact =
                            Literal::NegatedFunctionComparison(NegatedFunctionComparison {
        ...
        if fact.is_negated() {
            ...
            if let Some(atom) = fact.positive_atom() {
                ...
                if let Some(entries) = dictionary.get(&positive_fact) {
                    for (var, val) in entries {
                        let mut poss_vals: HashSet<usize> = (0..ranges[*var]).collect();
...
pub fn negate_and_translate_condition(
    ...
    for combination in combinations {
        let negated: Vec<Literal> = combination.iter().map(|lit| lit.negate()).collect();
    ...
    for (var, effects) in effects_by_variable {
        ...
        for (post, eff_conditions) in effects {
            ...
            let mut eff_condition_lists: Vec<Vec<(usize, usize)>> = eff_conditions
                ...
                .map(|eff_cond| {
                    let mut v: Vec<(usize, usize)> =
                        eff_cond.iter().map(|(k, v)| (*k, *v)).collect();
                    v.sort();
                    v
                })
                .collect();
            ...
            for eff_condition in eff_condition_lists {
                ...
                pre_post.push((var, pre, post, filtered));
                added_effect = true;
            }
        }
        if added_effect {
...
pub fn translate_numeric_axiom(
    ...
    for part in &axiom.parts {
        match part {
            ...
            crate::numeric_axiom_rules::NumericPart::Axiom(sub) => {
            ...
            crate::numeric_axiom_rules::NumericPart::Constant(constant) => {
                let key = format!(
                    "derived!{}()",
                    crate::function_expression::format_float(constant.0.into_inner())
                );
...
pub fn translate_task(
    ...
    let mut seen_numeric_axioms: HashSet<(String, Vec<usize>, usize)> = HashSet::new();
    let numeric_axioms: Vec<crate::sas::NumericAxiom> = num_axioms
        .iter()
