//! Wall-clock and memory limits: parsing them, imposing them on a child, and
//! turning the way a child died back into an exit code.
//!
//! The planner runs its own search in a child process precisely so that these
//! are enforceable -- `RLIMIT_CPU` and an RSS watchdog can stop a child, but
//! nothing can stop the process that is doing the searching from inside it.
...
use crate::output::{EXIT_OUT_OF_MEMORY, EXIT_TIMEOUT};
...
pub(crate) fn parse_memory_limit(input: &str) -> Result<u64, String> {
...
#[cfg(unix)]
pub(crate) fn effective_rss_limit(memory_limit: u64) -> std::io::Result<u64> {
...
pub(crate) fn parse_time_limit(input: &str) -> Result<Duration, String> {
...
pub(crate) fn format_time_limit(limit: Duration) -> String {
...
#[cfg(unix)]
pub(crate) fn normalize_wrapped_exit(
...
#[cfg(unix)]
pub(crate) fn apply_process_limits(
    time_limit: Option<Duration>,
    memory_limit: Option<u64>,
) -> std::io::Result<()> {
...
#[cfg(target_os = "linux")]
pub(crate) fn wait_with_memory_limit(
