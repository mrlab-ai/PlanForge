...
impl<'task> ProjectedTask<'task> {
    pub fn new(
        ...
        for &numeric_var_id in &pattern.numeric {
            ...
            {
                selected_helper_backed_vars.insert(auxiliary_numeric_var.source_numeric_var_id);
    ...
    pub fn apply_operator_to_state_in_place(
        ...
        operator_id: usize,
        successor_propositional: &mut Vec<usize>,
        ...
        for effect in &cached_operator.propositional_effects {
            if cached_facts_hold(propositional_values, &effect.conditions) {
                ...
                self.propositional_packer
                    .set(successor_packed, effect.var_id, effect.value as u64);
            }
        }
        ...
        (0..self.projected_num_var_to_original.len())
            .filter(|&projected_index| {
                ...
                self.projected_num_var_to_original[projected_index]
                    .and_then(|original_numeric_var| {
                        self.base.numeric_variables().get(original_numeric_var)
                    })
...
impl AbstractNumericTask for ProjectedTask<'_> {
    ...
    fn get_num_operator_preconditions(&self, index: usize, is_axiom: bool) -> usize {
        if is_axiom {
            self.axioms
                .get(index)
                .map(|axiom| axiom.conditions().len())
                .unwrap_or(0)
        } else {
            self.operators
                .get(index)
    ...
    fn get_num_operator_effects(&self, index: usize, is_axiom: bool) -> usize {
        if is_axiom {
            self.axioms.get(index).map(|_| 1).unwrap_or(0)
        } else {
            self.operators
                .get(index)
...
fn cache_facts(facts: &[ExplicitFact]) -> Vec<CachedFact> {
    facts
        .iter()
        .map(|fact| CachedFact {
            var_id: fact.var(),
...
fn cached_facts_hold(propositional: &[usize], facts: &[CachedFact]) -> bool {
    facts
        .iter()
        .all(|fact| propositional.get(fact.var_id).copied() == Some(fact.value))
}

fn cached_assignment_effect_holds(
    propositional: &[usize],
    effect: &CachedAssignmentEffect,
) -> bool {
        ...
        .map(|operator| CachedOperator {
            ...
            assignment_effects: operator
                ...
                .map(|effect| CachedAssignmentEffect {
                    conditions: cache_facts(effect.conditions()),
                    source_var_id: effect.var_id(),
                    target_var_id: effect.affected_var_id(),
...
fn project_operator(
    ...
    let preconditions: Vec<ExplicitFact> = operator
        .preconditions()
        .iter()
        .filter_map(|fact| project_fact(fact, var_map))
        .collect();
    ...
    if !auxiliary_numeric_vars.is_empty() {
        ...
        for effect in operator.assignment_effects() {
            ...
            match effect.operation() {
                planforge_sas::numeric::numeric_task::AssignmentOperation::Assign => {
                ...
                planforge_sas::numeric::numeric_task::AssignmentOperation::Minus => {
                    if source_type != &NumericType::Constant {
                        return Err(ProjectedTaskBuildError::UnsupportedOperatorEffect {
                            operator_name: operator.name().to_string(),
                            reason: "lifted numeric effects require constant right-hand sides",
                ...
                planforge_sas::numeric::numeric_task::AssignmentOperation::Times
                | planforge_sas::numeric::numeric_task::AssignmentOperation::Divide => {
                    return Err(ProjectedTaskBuildError::UnsupportedOperatorEffect {
                        operator_name: operator.name().to_string(),
        ...
        for auxiliary_numeric_var in auxiliary_numeric_vars {
            ...
            if !is_auxiliary_num_var
                .get(projected_target)
                .copied()
                .unwrap_or(false)
            {
                continue;
