...
use planforge_sas::numeric::numeric_task::{AbstractNumericTask, ExplicitFact, NumericType};
use planforge_sas::numeric::utils::errors::{AxiomEvalError, InvalidIndex};
...
pub fn progress(
    ...
    for eff in op.assignment_effects().iter() {
        ...
        numeric_state[affected_var_id] =
            planforge_sas::numeric::numeric_task::AssignmentOperation::apply(
                numeric_state[affected_var_id],
                eff.operation(),
                operand,
            );
