            ...
            for &pdb_id in subset {
                let value = if let Some(cached_value) = pdb_value_cache.get(pdb_id) {
                    ...
                    let value = pdbs[pdb_id]
                        .lookup_projected_or_fallback_from_source_state_values_fast(
...
impl<'task> CanonicalNumericPdbHeuristic<'task> {
    pub fn from_config(
        ...
        super::validate_restricted_task(task)?;
        ...
        Ok(Self::new(CanonicalPdbCollectionInformation::new(
            task,
            patterns,
    ...
    pub fn new(collection_information: CanonicalPdbCollectionInformation<'task>) -> Self {
        Self {
