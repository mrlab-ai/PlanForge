...
pub struct GlobalConstraint {
    pub var: *mut crate::preprocess_port::variable::Variable,
    pub val: i32,
}
...
fn read_mutexes(
    stream: &mut InputStream,
    mutexes: &mut Vec<crate::preprocess_port::mutex_group::MutexGroup>,
...
fn read_operators(
    ...
    operators: &mut Vec<crate::preprocess_port::operator::Operator>,
) {
    let count = stream.read_i32();
    for _ in 0..count {
        operators.push(crate::preprocess_port::operator::Operator::from_stream(
            stream,
            variables,
            numeric_variables,
        ));
...
fn read_axioms_rel(
    stream: &mut InputStream,
    variables: &Vec<*mut Variable>,
    axioms_rel: &mut Vec<crate::preprocess_port::axiom::AxiomRelational>,
) {
    let count = stream.read_i32();
    for _ in 0..count {
...
fn read_axioms_func_comp(
    ...
    variables: &mut Vec<*mut Variable>,
    numeric_variables: &Vec<*mut NumericVariable>,
    axioms_func_comp: &mut Vec<crate::preprocess_port::axiom::AxiomFunctionalComparison>,
) {
    let count = stream.read_i32();
    check_magic(stream, "begin_comparison_axioms");
    for _ in 0..count {
        axioms_func_comp.push(
            crate::preprocess_port::axiom::AxiomFunctionalComparison::from_stream(
                stream,
                variables,
                numeric_variables,
            ),
...
fn read_axioms_numeric(
    stream: &mut InputStream,
    numeric_variables: &mut Vec<*mut NumericVariable>,
    axioms_numeric: &mut Vec<crate::preprocess_port::axiom::AxiomNumericComputation>,
) {
    let count = stream.read_i32();
    check_magic(stream, "begin_numeric_axioms");
    for _ in 0..count {
        axioms_numeric.push(
            crate::preprocess_port::axiom::AxiomNumericComputation::from_stream(
                stream,
                numeric_variables,
            ),
        );
...
pub fn read_preprocessed_problem_description(
    ...
    numeric_variables: &mut Vec<*mut NumericVariable>,
    mutexes: &mut Vec<crate::preprocess_port::mutex_group::MutexGroup>,
    initial_state: &mut crate::preprocess_port::state::State,
    goals: &mut Vec<(*mut Variable, i32)>,
    operators: &mut Vec<crate::preprocess_port::operator::Operator>,
    axioms_rel: &mut Vec<crate::preprocess_port::axiom::AxiomRelational>,
    axioms_func_ass: &mut Vec<crate::preprocess_port::axiom::AxiomNumericComputation>,
    axioms_func_comp: &mut Vec<crate::preprocess_port::axiom::AxiomFunctionalComparison>,
