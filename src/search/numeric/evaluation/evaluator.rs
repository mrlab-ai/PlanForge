...
pub struct EvaluationState {
    /// The evaluation result being built
...
impl EvaluationState {
    /// Creates a new evaluation state for the given state and g-value
    ...
    pub fn is_computed(&self, evaluator_name: &str) -> bool {
        self.computed_evaluators.borrow().get(evaluator_name).copied().unwrap_or(false)
    }

    /// Marks an evaluator as computed
    pub fn mark_computed(&self, evaluator_name: &str) {
        self.computed_evaluators.borrow_mut().insert(evaluator_name.to_string(), true);
    }
    ...
    pub fn get_or_compute_heuristic<E: Evaluator + ?Sized>(
        ...
        self.result.set_heuristic_value(evaluator_name.clone(), value);
...
pub trait Evaluator {
    ...
    fn evaluate(&self, state: &ConcreteState, g_value: f64) -> Result<EvaluationResult, EvaluationError> {
...
impl EvaluatorCollection {
    ...
    pub fn evaluate_all(&self, state: &ConcreteState, g_value: f64) -> Result<EvaluationResult, EvaluationError> {
        let mut eval_state = EvaluationState::new(state.clone(), g_value, false);
...
mod tests {
    ...
    impl Evaluator for TestEvaluator {
        ...
        fn evaluate_state(&self, eval_state: &mut EvaluationState) -> Result<f64, EvaluationError> {
            eval_state.result_mut().set_heuristic_value(self.name(), self.value);
            Ok(self.value)
        }
    }
