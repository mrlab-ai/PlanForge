...
use planners_sas::numeric::numeric_task::{
	AbstractNumericTask, AssignmentEffect, AssignmentOperation, Fact, NumericType, Operator,
};
...
pub(crate) struct NumericTaskHelper {
	...
	fact_to_axiom_marker: Vec<i32>,
	numeric_variable_ids: Vec<usize>,
	numeric_variable_index_by_task_id: Vec<i32>,
	comparison_axiom_by_var: BTreeMap<usize, usize>,
	comparison_fact_condition_group_ids: BTreeMap<(usize, i32), Vec<usize>>,
	comparison_fact_conditions: BTreeMap<(usize, i32), Vec<LinearNumericCondition>>,
	goal_helper_propositional_facts: BTreeMap<usize, Vec<Fact>>,
...
pub(crate) struct HelperPreconditionLists {
	pub(crate) propositional_facts: Vec<Fact>,
	pub(crate) numeric_group_ids: Vec<usize>,
}

#[derive(Debug, Clone)]
pub(crate) struct HelperConditionalFactEffect {
	pub(crate) preconditions: HelperPreconditionLists,
	pub(crate) add_fact: Fact,
	pub(crate) del_fact: Option<Fact>,
}
...
pub(crate) struct HelperActionModel {
	pub(crate) propositional_preconditions: Vec<Fact>,
	pub(crate) numeric_precondition_group_ids: Vec<usize>,
	pub(crate) add_facts: Vec<Fact>,
	pub(crate) del_facts: Vec<Fact>,
	pub(crate) pre_del_facts: Vec<Fact>,
	pub(crate) conditional_fact_effects: Vec<HelperConditionalFactEffect>,
...
impl NumericTaskHelper {
	pub(crate) fn new(
		...
		let mut helper = Self {
			fact_to_axiom_marker: vec![
				-1;
				usize::try_from(task.get_num_variables().max(0)).unwrap_or(0)
			],
			numeric_variable_index_by_task_id: vec![-1; task.numeric_variables().len()],
			..Self::default()
		};
		...
		for (local_var_id, &task_var_id) in helper.numeric_variable_ids.iter().enumerate() {
			if let Some(index) = helper.numeric_variable_index_by_task_id.get_mut(task_var_id) {
				*index = i32::try_from(local_var_id).expect("numeric local variable id must fit i32");
			}
		}
	...
	pub(crate) fn get_action_pre_list(&self, action_id: usize) -> Option<&[Fact]> {
	...
	pub(crate) fn get_action_add_list(&self, action_id: usize) -> Option<&[Fact]> {
		self.action_model(action_id)
	...
	pub(crate) fn get_action_conditional_fact_effects(
		&self,
		action_id: usize,
	) -> Option<&[HelperConditionalFactEffect]> {
		self.action_model(action_id)
	...
	pub(crate) fn get_propositional_goals(&self, goal_id: usize) -> Option<&[Fact]> {
	...
	pub(crate) fn linearized_effect_for_action_assignment(
		...
		let linear_effect = self.linear_effect_for_assignment_effect(action_id, assignment_effect_id)?;
	...
	pub(crate) fn linearized_effects_for_action(
		...
		let Some(action_model) = self.action_model(action_id) else {
			return Err(format!("numeric helper action model {action_id} is missing"));
		};
		...
		for linear_effect in &action_model.linear_effects {
			...
			effects[assignment_effect_id] = self
				.linearized_effect_for_action_assignment(action_id, assignment_effect_id);
		}
	...
	pub(crate) fn proposition_fact(&self, proposition_id: usize) -> Option<&Fact> {
	...
	pub(crate) fn fact_to_axiom_marker(&self, variable_id: usize) -> Option<i32> {
	...
	pub(crate) fn goal_materialized_conditions(
		...
		self.goal_model(goal_id)
			.map(|goal_model| self.materialized_conditions_for_group_ids(&goal_model.numeric_group_ids))
			.unwrap_or_default()
	}
	...
	pub(crate) fn condition_group_representative_condition_id(&self, group_id: usize) -> Option<usize> {
		self.condition_group_representative_condition_ids.get(group_id).copied()
	}

	pub(crate) fn normalized_condition(&self, condition_id: usize) -> Option<&LinearNumericCondition> {
	...
	pub(crate) fn build_precondition_lists(
		...
		for condition in preconditions {
			let var_id = condition.var() as usize;
	...
	pub(crate) fn build_base_precondition_lists_with_redundancy(
		...
		let (propositional_facts, _numeric_group_ids) = self.build_precondition_lists(preconditions);
	...
	pub(crate) fn build_conditional_precondition_lists_with_redundancy(
		...
		let base_numeric_group_ids =
			self.build_numeric_precondition_group_ids_with_redundancy(base_preconditions, precision);
		let (propositional_facts, _local_numeric_group_ids) = self.build_precondition_lists(conditions);
	...
	pub(crate) fn condition_group_ids_for_numeric_fact(&self, fact: &Fact) -> Vec<usize> {
	...
	pub(crate) fn build_numeric_precondition_group_ids_with_redundancy(
		...
		let (_propositional_facts, numeric_group_ids) = self.build_precondition_lists(preconditions);
	...
	pub(crate) fn build_conditional_numeric_group_ids_with_redundancy(
		...
		let (_propositional_facts, local_numeric_group_ids) = self.build_precondition_lists(conditions);
		let mut result = local_numeric_group_ids.clone();
		let pairwise_group_ids =
			self.materialize_pairwise_redundant_condition_groups(&local_numeric_group_ids, precision);
			...
			for right_index in (left_index + 1)..group_ids.len() {
				...
				for left_condition_id in left_condition_ids {
					for &right_condition_id in &right_condition_ids {
						let Some(left_condition) = self.normalized_condition(left_condition_id).cloned() else {
							continue;
						};
	...
	fn build_propositions(&mut self, task: &dyn AbstractNumericTask) {
		...
		for var_id in 0..usize::try_from(task.get_num_variables().max(0)).unwrap_or(0) {
			...
			.expect("helper proposition domain size must fit usize");
			...
			for value in 0..domain_size {
				...
				let fact = Fact::new(var_id as u32, value as i32);
	...
	fn build_mutex_actions(&mut self, task: &dyn AbstractNumericTask) {
		...
		let num_variables = usize::try_from(task.get_num_variables().max(0)).unwrap_or(0);
		self.mutex_action_ids = vec![Vec::new(); operators.len()];
		for (op_id, operator) in operators.iter().enumerate() {
			let mut precondition = vec![-1; num_variables];
			let mut postcondition = vec![-1; num_variables];
			for condition in operator.preconditions() {
				precondition[condition.var() as usize] = condition.value();
			}
			for effect in operator.effects() {
				postcondition[effect.var_id() as usize] = effect.value() as i32;
			}
			...
			for (other_op_id, other_operator) in operators.iter().enumerate() {
				...
				for effect in other_operator.effects() {
					...
					let post = effect.value() as i32;
					if (precondition[var_id] != post && precondition[var_id] != -1)
						|| (postcondition[var_id] != post)
					{
						is_mutex = true;
						break;
					}
	...
	fn build_action_from_operator(
		...
		let base_preconditions = self.build_base_precondition_lists_with_redundancy(
		...
		for precondition in operator.preconditions() {
			base_precondition_values.insert(precondition.var() as usize, precondition.value());
		}

		for effect in operator.effects() {
			let add_fact = Fact::new(effect.var_id(), effect.value() as i32);
			if effect.conditions().is_empty() {
				action_model.add_facts.push(add_fact.clone());
				if let Some(&pre_value) = base_precondition_values.get(&(effect.var_id() as usize)) {
					action_model.del_facts.push(Fact::new(effect.var_id(), pre_value));
				}
			} else {
				let conditional_preconditions = self.build_conditional_precondition_lists_with_redundancy(
					operator.preconditions(),
					effect.conditions(),
					precision,
				);
				...
				for condition in effect.conditions() {
					extended_precondition_values
						.insert(condition.var() as usize, condition.value());
				}
				let del_fact = extended_precondition_values
					.get(&(effect.var_id() as usize))
					.copied()
					.map(|pre_value| Fact::new(effect.var_id(), pre_value));
				action_model
					.conditional_fact_effects
					.push(HelperConditionalFactEffect {
		...
		let linearized_assignment_effects = task
			...
			.unwrap_or_else(|error| {
				panic!(
					"failed to build helper action model for operator {operator_id}: {error}"
				)
			});
	...
	fn build_action_from_axiom(
		...
		action_model
			.add_facts
			.push(Fact::new(axiom.var_id(), axiom.effect_value() as i32));
		action_model.del_facts.push(Fact::new(
			axiom.var_id(),
			axiom.precondition_value() as i32,
		));
		action_model.pre_del_facts = intersect_fact_lists(
			&action_model.propositional_preconditions,
			&action_model.del_facts,
	...
	fn classify_assignment_effect_into_action(
		...
		let conditional_preconditions = if assignment_effect.is_conditional()
			|| !assignment_effect.conditions().is_empty()
		{
		...
		action_model.linear_effects.push(HelperLinearEffect {
			source_assignment_effect_id: assignment_effect_id,
			source_var_id: assignment_effect.var_id() as usize,
			...
			for (condition_id, condition) in self.normalized_conditions.iter().enumerate() {
				let cumulative_effect = self
					...
					.map(|(local_var_id, &task_var_id)| {
						condition.coefficients[task_var_id] * action_model.simple_effects[local_var_id]
					})
					.sum::<f64>();
	...
	fn calculates_bounds_numeric_variables(
		...
		self.numeric_variable_lower_bounds = self
			.numeric_variable_ids
			.iter()
			.map(|&task_var_id| initial_numeric_values.get(task_var_id).copied().unwrap_or(0.0))
			.collect();
	...
	fn build_numeric_conditions(&mut self, task: &dyn AbstractNumericTask) {
		for (comparison_axiom_id, comparison_axiom) in task.comparison_axioms().iter().enumerate() {
			...
			if let Some(marker) = self.fact_to_axiom_marker.get_mut(affected_var_id) {
				*marker = i32::try_from(comparison_axiom_id)
					.expect("comparison axiom id must fit i32");
			}
			...
			// lookup and filters unsupported disequality facts earlier.
	...
	fn build_numeric_goals(&mut self, task: &dyn AbstractNumericTask, precision: f64) {
		self.goal_models = vec![HelperPreconditionLists::default(); usize::try_from(task.get_num_goals().max(0)).unwrap_or(0)];
		let mut axiom_table: BTreeMap<usize, Vec<usize>> = BTreeMap::new();
		let mut fact_table: BTreeMap<usize, Vec<Fact>> = BTreeMap::new();

		for axiom in task.axioms() {
			let effect_var_id = axiom.var_id() as usize;
			for precondition in axiom.conditions() {
				let precondition_var_id = precondition.var() as usize;
...
fn intersect_fact_lists(left: &[Fact], right: &[Fact]) -> Vec<Fact> {
	let right_set = right
		.iter()
...
fn final_expression_from_effect(linear_effect: &LinearNumericEffect) -> LinearExpression {
	...
	if let Some(coefficient) = expression.coefficients.get_mut(linear_effect.affected_var_id) {
		*coefficient += 1.0;
	}
	expression
}
