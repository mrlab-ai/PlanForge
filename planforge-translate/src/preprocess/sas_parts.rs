//! Phrasing the reordered task in the shape the SAS+ format carries.
//!
//! The one traversal of a [`ReorderedTask`]: every variable reference in it is
//! still phrased in the id the variable had before the reordering, and is
//! remapped to its level here. What comes out is
//! [`planforge_sas::sas_format::SasTaskParts`], which is either written as the
//! SAS+ file another planner reads or turned into the task this process
//! searches. Both consumers therefore see the same task by construction, rather
//! than by two traversals agreeing.
//!
//! What the format leaves implicit — the axiom default of a derived variable, the
//! merge of an operator's two condition lists — belongs to
//! `planforge_sas::sas_format` and is applied on the way out of the parts, so
//! this module states none of it.
...
use crate::sas_tasks::SasFact;

pub fn build(reordered: &ReorderedTask) -> SasTaskParts {
    let ReorderedTask {
...
fn build_operator(reordered: &ReorderedTask, op: &crate::sas_tasks::SASOperator) -> SasOperator {
    let fact = |pair: &SasFact| {
        let (var, value) = reordered.prop_fact(pair);
        ExplicitFact::propositional(var, value)
    };
