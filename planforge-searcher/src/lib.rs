//! Search-spec parsing and heuristic construction for the `planforge` binary.
//!
//! This crate owns the `--search` grammar and turns a parsed [`HeuristicSpec`]
//! into a `Box<dyn Heuristic>`. It deliberately owns no CLI, no process
//! plumbing and no output formatting: those live in the `planforge` binary
//! crate, which is the single entry point every search runs through.
